        ...
        [
            ...
            html.Div(
                [
                    ...
                    html.Span(
                        "AI Data Supply Chain Trust & Provenance",
    ...
    cards = html.Div(
        [
            ...
            _card(
                "Integrity",
                verify["integrity"],
                "#2ecc71" if verify["integrity"] == "OK" else "#e74c3c",
            ),
            _card(
                "Failed", str(verify["failed"]), "#e74c3c" if verify["failed"] > 0 else "#2ecc71"
            ),
        ],
    ...
    return html.Div(
        [
            ...
            dash_table.DataTable(
                data=data,
                columns=[{"name": col, "id": col} for col in data[0]] if data else [],
            ...
            if data
            else html.P("No records yet.", style={"color": "#888"}),
        ]
    )
...
def _build_compliance(tp: TrustPipe) -> html.Div:
    ...
    return html.Div(
        [
            ...
            dash_table.DataTable(
                data=rows,
                columns=[{"name": col, "id": col} for col in rows[0]] if rows else [],
                style_table={"overflowX": "auto"},
                style_cell={"textAlign": "left", "padding": "8px", "fontSize": "13px"},
                style_header={"fontWeight": "bold", "backgroundColor": "#f8f9fa"},
                style_data_conditional=[
            ...
            if rows
            else html.P("No datasets tracked yet.", style={"color": "#888"}),
        ]
    )
...
def _card(title: str, value: str, color: str) -> html.Div:
    return html.Div(
        [
            html.Div(
                title, style={"fontSize": "12px", "color": "#888", "textTransform": "uppercase"}
            ),
